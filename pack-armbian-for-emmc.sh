...
# touchscreen never creates an input device. This hex string was previously truncated
# to 180 bytes (missing zero-padding bytes before the descending keymap sequence, an
# indistinguishable-by-eye copy/paste error) -- confirmed live: /lib/firmware/
# goodix_911_cfg.bin was 180 bytes on-device even after fixing the (different, also
# truncated) copy in userpatches-chroot/20-rk3308bs-hardware.sh, because THIS script
# runs later, directly writing into the final mounted rootfs partition, and always
# wins as the last writer. Re-derived from the verified-correct 186-byte
